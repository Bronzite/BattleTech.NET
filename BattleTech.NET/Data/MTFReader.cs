    ...
    public static class MTFReader
    {
        ...
        public static BattleMechDesign ReadBattleMechDesignFile(Stream inStream)
        {
            ...
            try
            {
                ...
                if (dVersion<1.3)
                {
                    ...
                    for (int i = 3; i < sLines.Length; i++)
                    {
                        if (sLines[i] != "" && !bTextMode)
                        {
                            ...
                            {
                                ConfigureBattleMechDesign(kvp.Value,retval);
                            }
                            ...
                            if (kvp.Key.Equals("Mass", StringComparison.CurrentCultureIgnoreCase))
                            {
                                ...
                                if (retval.Tonnage > 200) throw new DesignUnsupportedTypeException("Ultraheavy");
                            }
                                    ...
                                    for (int j = 0; j<CriticalHitSlotCount[sKey]; j++)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            foreach (ComponentAmmunition componentAmmunition in ComponentLibrary.Ammunitions)
                                            {
                                                ...
                                                {
                                                    criticalSlot.AffectedComponent = new UnitComponent(componentAmmunition.Clone() as Common.Component, selectedLocation);
                        ...
                        foreach (BattleMechHitLocation bmhl in retval.HitLocations)
                        {
                            if (bmhl.CriticalSlots != null)
                                foreach (CriticalSlot criticalSlot in bmhl.CriticalSlots)
                                {
                                    ...
                                    foreach (Common.Component weapon in ComponentLibrary.Weapons.Values)
                                    {
                ...
                if (dVersion==3)
                {
                    List<KeyValuePair<string,string>> dicElements = new List<KeyValuePair<string, string>>();
                    KeyValuePair<string, string>? curKVP = null;
                    string sText = "";
                    string sLabel = "";
                    int iHeatDissipation = 0;

                    for (int i = 0; i <  sLines.Length; i++)
                    {
                        if(sLines[i].Contains(":"))
                        {
                            int iColonLocation = sLines[i].IndexOf(':');
                            string Label = sLines[i].Substring(0, iColonLocation);
                            string Value = sLines[i].Substring(iColonLocation + 1).Trim();
                            if (sLabel != "")
                            {
                                dicElements.Add(new KeyValuePair<string, string>(sLabel,sText));
                                sLabel="";
                                sText="";
                            }
                            sLabel = Label;
                    ...
                    if(sLabel != "") dicElements.Add(new KeyValuePair<string, string>(sLabel, sText));

                    foreach (KeyValuePair<string,string> kvp in dicElements)
                    {
                        string sKey = kvp.Key;
                        string sValue = kvp.Value;
                        if(sKey.Equals("config", StringComparison.CurrentCultureIgnoreCase))
                        {
                            ConfigureBattleMechDesign(sValue, retval);
                        }
                        if (sKey.Equals("chassis", StringComparison.CurrentCultureIgnoreCase)) retval.Model = sValue;
                        if (sKey.Equals("model", StringComparison.CurrentCultureIgnoreCase)) retval.Variant = sValue;
                        if (sKey.Equals("TechBase",StringComparison.CurrentCultureIgnoreCase))
                        {
                            if (Utilities.IsSynonymFor(sValue, "Inner Sphere"))
                            {
                                retval.TechnologyBase = TECHNOLOGY_BASE.INNERSPHERE;
                            }
                            if (Utilities.IsSynonymFor(sValue, "Clan"))
                            {
                                retval.TechnologyBase = TECHNOLOGY_BASE.CLAN;
                            }
                            if (Utilities.IsSynonymFor(sValue, "Mixed"))
                            {
                                retval.TechnologyBase = TECHNOLOGY_BASE.BOTH;
                                retval.MixedTechBase = true;
                            }
                        }
                        if (sKey.Equals("Mass", StringComparison.CurrentCultureIgnoreCase))
                        {
                            int iMass = 0;
                            if (!int.TryParse(sValue, out iMass))
                            {
                                throw new Exception(string.Format("Unable to parse mass: {0}", sValue));
                            }
                            retval.Tonnage = iMass;
                            if (retval.Tonnage > 200) throw new DesignUnsupportedTypeException("Ultraheavy battlemechs not supported");
                        }
                        if (sKey.Equals("Engine", StringComparison.CurrentCultureIgnoreCase))
                        {
                            string[] strings = sValue.Split(' ', 2);
                            int iRating = 0;
                            if (!int.TryParse(strings[0], out iRating))
                            {
                                throw new Exception(string.Format("Unable to parse engine rating: {0}", strings[0]));
                            }
                            ComponentEngine engine = new ComponentEngine(iRating, strings[1]);
                            retval.Engine = engine;
                        }
                        if (sKey.Equals("Structure", StringComparison.CurrentCultureIgnoreCase))
                        {
                            foreach (StructureType curStructureType in StructureType.GetCanonicalStructureTypes())
                            {
                                if (Utilities.IsSynonymFor(curStructureType, sValue) &&
                                   retval.IsCompatible(curStructureType))
                            ...
                            if (retval.StructureType == null) throw new Exception(string.Format("Cannot find structure type: {0}", sValue));
                        }
                        if (sKey.Equals("Myomer", StringComparison.CurrentCultureIgnoreCase))
                        {
                            foreach (MyomerType curMyomerType in MyomerType.GetCanonicalMyomerTypes())
                            {
                                if ((curMyomerType.Name.Equals(sValue, StringComparison.CurrentCultureIgnoreCase) ||
                                    Utilities.IsSynonymFor(curMyomerType, sValue)) &&
                                    retval.IsCompatible(curMyomerType)
                                    )
                                {
                                    retval.MyomerType = curMyomerType;
                            ...
                            if (retval.MyomerType == null) throw new Exception(string.Format("Cannot find Myomer type: {0}", sValue));
                        }
                        if (sKey.Equals("Armor", StringComparison.CurrentCultureIgnoreCase))
                        {
                            foreach (ArmorType curArmorType in ArmorType.CanonicalArmorTypes())
                            {
                                if (Utilities.IsSynonymFor(curArmorType, sValue) &&
                                    retval.IsCompatible(curArmorType)
                                    )
                                {
                                    foreach (BattleMechHitLocation hitLocation in retval.HitLocations)
                        ...
                        if (sKey.Contains(" Armor", StringComparison.CurrentCultureIgnoreCase))
                        {
                            int iArmorAmount = 0;
                            if (!int.TryParse(sValue, out iArmorAmount))
                                throw new Exception($"Unable to get value from {sKey}:{sValue}.");
                            string sArmorLocationKey = sKey.Replace(" Armor", "").Replace(" armor", "").Trim();
                            foreach (BattleMechHitLocation bmhl in retval.HitLocations)
                            {
                                if (bmhl.ArmorFacings.ContainsKey(sArmorLocationKey))
                                {
                        ...
                        if(sKey.Equals("Heat Sinks",StringComparison.CurrentCultureIgnoreCase))
                        {
                            string[] stringProperties = sValue.Split(' ');
                            retval.HeatDissipation = int.Parse(stringProperties[0]);
                            retval.DoubleIntegralHeatSinks  = stringProperties[1].Equals("Double", StringComparison.CurrentCultureIgnoreCase);
                        }

                        if (retval.HitLocations != null)
                        {
                            foreach (BattleMechHitLocation bmhl in retval.HitLocations)
                            {
                                if (Utilities.IsSynonymFor(bmhl.Name, sKey))
                                {
                                    string[] sSlots = sValue.Split("\r\n",StringSplitOptions.RemoveEmptyEntries);
                                    //if (sSlots.Length != bmhl.CriticalSlotCount) throw new Exception($"Critical Slot Count Mismatch: {bmhl.CriticalSlotCount} slots expected, {sSlots.Length} slots found.");
                                    
                                    for (int i = 0; i<sSlots.Length; i++)
                                    {
                                        bmhl.AddCriticalSlot(new CriticalSlot() { Label = sSlots[i], Location = bmhl, SlotNumber = i+1 });
                                    }
                                }
                            }
                        }
                    ...
                    //Install Gyro
                    ComponentGyro.ResolveComponents(retval);

                    //Install Cockpit
                    ComponentCockpit.ResolveComponents(retval);

                    //Install Weapons
                    ComponentWeapon.ResolveComponent(retval);
                    ...
                    ComponentCASE.ResolveComponent(retval);
                    ComponentRetractableBlade.ResolveComponents(retval);
                    ComponentHeatSink.ResolveComponents(retval);
                    ComponentAmmunition.ResolveComponents(retval);
                }
            }
            catch (Exception ex)
            {
        ...
        static private void ConfigureBattleMechDesign(string sConfig, BattleMechDesign retval)
        {
            if(retval.HitLocations==null) retval.HitLocations = new List<HitLocation>();
                ...
                if (sConfig.Equals("Biped", StringComparison.InvariantCultureIgnoreCase) ||
                    ...
                    sConfig.Equals("LAM", StringComparison.InvariantCultureIgnoreCase)
                    )
                {
            ...
            if (sConfig.Equals("Tripod", StringComparison.InvariantCultureIgnoreCase) ||
               sConfig.Equals("Tripod OmniMech", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                BattleMechHitLocation mhlCenterLeg = new BattleMechHitLocation()
                {
                    Name = "CL",
                    ArmorFacings = new Dictionary<string, ArmorFacing>()
                                    {
                                        { "CL", new ArmorFacing() { Name = "LL" } }
                                    },
                    CriticalSlotCount = CriticalHitSlotCount["Center Leg"],
                    Structure = new StructureLocation()
                    {
                ...
                retval.HitLocations.Add(mhlRightArm);
                retval.HitLocations.Add(mhlLeftLeg);
                retval.HitLocations.Add(mhlCenterLeg);
                retval.HitLocations.Add(mhlRightLeg);
            }
            ...
            if (sConfig.Equals("Quad", StringComparison.InvariantCultureIgnoreCase)||
                    ...
                    sConfig.Equals("QuadVee", StringComparison.InvariantCultureIgnoreCase))
                {
                    BattleMechHitLocation mhlHead = new BattleMechHitLocation()
                    {
                        Name = "HD",
